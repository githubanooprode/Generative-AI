function [settings,currentStep] = FillIn_ShiftWholeImage_withBlend(settings,currentStep)
...
offsetY=-currentStep.sourcePatchY(1) + currentStep.targetPatchY(1);
offsetX=-currentStep.sourcePatchX(1) + currentStep.targetPatchX(1);
...
if strcmp(settings.blendDistanceFunction, 'getBlendPriorityOnlyOutsideTarget')
    ...
    settings.initialLabImage = ...
        ...
        %(initialLabImage.* unknown_area .* shifted_amoeba_map)*(dont_blend) + ...

else
    %blendPriority = currentStep.blendDistanceFunction(currentStep.targetArea3Color, currentStep.confidence3Color,currentStep);
    %settings.blend=blendPriority*0;
    settings.blend=ones(size(currentStep.targetArea3Color))*.25;
    dont_blend=1-settings.blend;
       tmpImage= ...
        (settings.initialLabImage.* known_area .* ~currentStep.shiftedAmoebaMap) + ... %if no amoeba here in the known region, then use original image in full
        (settings.initialLabImage.* known_area .* currentStep.shiftedAmoebaMap).*(dont_blend) + ... %if there is an amoeba here in the known region,  blend
        (shifted_source_patch.* known_area .* currentStep.shiftedAmoebaMap).*settings.blend + ... 
        shifted_source_patch.* unknown_area .* currentStep.shiftedAmoebaMap;
        settings.initialLabImage=tmpImage;
end
...
%figure(14); imshow(settings.blend(:,:,1))
