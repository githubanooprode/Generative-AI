function [gradient_angle,gradient_magnitude,gradient_angle_all,gradient_magnitude_all,voidBit]= calculateDominantPatchAngle_separateColor(targetPatchesY,targetPatchesX,currentStep,settings)
...
for myColor = 1:3
    [jj]=find(finalNN(:,myColor)==max(finalNN(:,myColor)));
    if isempty(patch_gradientDir3(:,myColor))
        gradient_angle(myColor)=0;
        gradient_magnitude(myColor)=0;
    else %if there is a tie, take the first one.
        gradient_angle(myColor)=hh(jj(1));
        if settings.useGradientMagAverage
            gradient_magnitude(myColor)=max(averagedM1(jj(1),myColor));
            %gradient_magnitude3=mean(averagedM1(jj(1),:));
        else
            gradient_magnitude(myColor)=max(finalSUM(jj(1),myColor));
...
%collapse across color channels
ghostRider=sum(finalNN,2);
[jj]=find(ghostRider==max(ghostRider));
if isempty(patch_gradientDir3)
    gradient_angle_all=0;
    gradient_magnitude_all=0;
else %if there is a tie, take the first one.
    gradient_angle_all=hh(jj(1));
    if settings.useGradientMagAverage
        gradient_magnitude_all=max(averagedM1(jj(1),:));
        %gradient_magnitude3=mean(averagedM1(jj(1),:));
    else
        gradient_magnitude_all=max(finalSUM(jj(1),:));
        %gradient_magnitude3=mean(finalNN(jj(1),:));
    end
end
